(function () {
  var WALLET_ICON = '<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-indigo-300" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z" /></svg>';
  ...
  var HOME_ICON  = '<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" /></svg>';
  ...
  var INACTIVE = 'nav-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800 transition-colors';
  var ACTIVE   = 'nav-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium bg-indigo-500 text-white transition-colors';
  ...
  container.innerHTML =
    '<aside class="fixed top-0 left-0 h-screen w-64 bg-slate-900 flex flex-col border-r border-slate-800 z-40">' +

      '<div class="p-5 border-b border-slate-800">' +
        '<a href="index.html" class="flex items-center gap-3">' +
          '<div class="w-9 h-9 rounded-xl bg-indigo-500/20 flex items-center justify-center flex-shrink-0">' + WALLET_ICON + '</div>' +
          '<div>' +
            '<div class="text-white font-bold text-lg leading-tight">Spendly</div>' +
            '<div class="text-slate-500 text-xs leading-tight">your money\'s smarter friend</div>' +
          '</div>' +
        '</a>' +
      '</div>' +

      '<nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">' +
        navLink('index.html',     'index.html',     HOME_ICON,   'Landing Page') +
      ...
      '<div id="sidebar-user-card" class="p-4 border-t border-slate-800">' +
        '<div class="flex items-center gap-3">' +
          '<div id="sidebar-avatar" class="w-10 h-10 rounded-full bg-indigo-500 flex items-center justify-center text-white text-sm font-bold flex-shrink-0 overflow-hidden">' +
            '<span id="sidebar-initials"></span>' +
          '</div>' +
          '<div class="min-w-0 flex-1">' +
            '<div id="sidebar-username" class="text-white text-sm font-semibold truncate">...</div>' +
            '<div class="text-slate-500 text-xs">Free plan</div>' +
          '</div>' +
          '<a href="profile.html" class="text-slate-400 hover:text-white flex-shrink-0" title="Settings">' + GEAR_ICON + '</a>' +
        '</div>' +
      '</div>' +
